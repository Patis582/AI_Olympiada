...
KLÍČOVÉ PRINCIPY:
1. Záznamy z anamnézy a epidemiologie jsou k dispozici – ale VŽDY doplň otázkami, protože pacientův subjektivní popis může odhalit důležité nuance
2. Vždy se zeptej na: intenzitu obtíží, délku trvání, zda se stav zhoršuje nebo zlepšuje
...
4. Netypické projevy (infarkt bez klasické bolesti na hrudi u diabetiků, žen, seniorů) vyžadují aktivní dotazování
5. Při nejistotě se raději zeptej než hádej

PRAVIDLO PRO OTÁZKY:
- V prvním kole se VŽDY zeptej – nikdy nepřiřazuj prioritu bez alespoň jednoho kola otázek
- Každé kolo klaď 2 otázky s možnostmi A/B/C/D
- Po 2 kolech otázek (4 otázky celkem) urči prioritu

FORMÁT ODPOVĚDI – VŽDY vrať POUZE validní JSON, žádný jiný text:

Pokud kladeš otázky (2 otázky za kolo):
{
  "action": "ask",
  "questions": [
    {
...
Pokud máš dost informací (po min. 1 kole otázek):
{
  "action": "prioritize",
  "priority": <číslo 1-5>,
...
def run_intake(pres: dict, hist: Optional[dict], epi: Optional[dict]) -> tuple:
    ...
    for round_num in range(3):  # 2 kola otázek + 1 finální
        context = _build_context(pres, hist, epi, qa_history)
        # První kolo vždy ask, po 2 kolech otázek force prioritize
        force = (round_num >= 2)
        result = _call_gpt(context, force_prioritize=force)

        if result.get("action") == "prioritize" and round_num >= 1:
